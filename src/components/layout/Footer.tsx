...
export function Footer() {
  ...
  return (
    <footer className="bg-[#181b28] text-white">
      <div className="mx-auto grid max-w-7xl gap-10 px-4 py-16 lg:grid-cols-4 lg:px-8">
        <div>
          <p className="mb-2 text-lg font-bold">{dict.footer.hotline}</p>
          <p className="mb-6 text-2xl font-bold text-[#fa561d]">
            {siteConfig.phones.join(" ")}
          </p>
          <Link
            href={dict.locale === "en" ? "/en/contact" : "/contact"}
            className="group inline-flex items-center gap-2 rounded-full border border-white/20 px-6 py-3 text-sm transition hover:border-[#fa561d] hover:bg-[#fa561d]"
          >
        ...
        <div>
          <h3 className="mb-5 text-lg font-bold">{dict.footer.categories}</h3>
          <ul className="space-y-3">
            {dict.categories.map((cat) => (
              <li key={cat.slug}>
                <Link
                  href={`${dict.locale === "en" ? "/en" : ""}/products/${cat.slug}`}
                  className="flex items-center gap-2 text-gray-300 transition hover:translate-x-1 hover:text-[#fa561d]"
                >
        ...
        <div>
          <h3 className="mb-5 text-lg font-bold">{dict.footer.contact}</h3>
        ...
        <div>
          <h3 className="mb-5 text-lg font-bold">{dict.footer.quickLinks}</h3>
          <ul className="space-y-3">
            {[
              ...
              <li key={link.href}>
                <Link href={link.href} className="text-gray-300 transition hover:text-[#fa561d]">
                  {link.label}
                </Link>
              </li>
            ))}
      ...
      <div className="border-t border-white/10">
        <div className="mx-auto px-4 py-6 text-center text-sm text-gray-400 lg:px-8">
